...
relevant_data = datasets[['d7normreg']]
...
shinyServer(function(input, output,session) {
  ...
  observeEvent(c(input$Dataset,input$MinUsers, input$AvgUsers, input$Aggregation),{
    ...
    updateSelectInput(session,'detailsWebsite',  choices = unique((Datasets[['RawData']])$SystemCode))
  })
  ...
  observeEvent(c(input$Contrast,Datasets),{
    ...
    observeEvent(input$updateButton,{
      ...
      if (input$overlays ==3)
      {
          ...
          cols_ds = colnames(Datasets[['relevant_data']]) %in% filt_chars
          cols_RD = colnames(Datasets[['RawData']]) %in% c(filt_chars,'sequence','SystemCode')
          tempData = Datasets[['relevant_data']]
    ...
    observeEvent(input$updateButton,
                  {
                    if (input$overlays == 2){
                      ...
                      selected_MT = Datasets[['relevant_data']][,cols_ds]
                      ...
                      if ((dim(selected_MT)[1] > 1) && (dim(selected_MT)[2] > 1))
                        ...
                        output$DistanceDistance = renderPlot({plot(x,which='dd', labs = rownames(selected_MT))})
    ...
    observeEvent(input$updateButton,
                 {
                   if (input$overlays == 1){
                   relevant_data =  Datasets[['relevant_data']]
                   RawData =  Datasets[['RawData']]
                   #print(head(relevant_data))
                   pars = getParameterName(Operator)
                   basis = getSelectedCharacteristics(Operator)

                   Operator = setSelection(Operator, relevant_data, input)
                   selected = getSelection(Operator)

                   #basis = getSelectedCharacteristics(Operator)
                   #pars  = getParameterName(Operator)
                   
                   tryCatch({ selected = selected[order(selected$impervious,decreasing=T),]},error=function(error_message){})
